...
Prior to the probability calculation code being created, the game history was filled with games containing randomly selected moves that missed easy wins or imminent losses.  This filled the training data with poor choices that were being repeated over and over.  To temporarily rectify this, the vector states are searched for a win that can be achieved in the current move or a loss that could be avoided in the next (opponent) move.  If a win can be achieved, or a loss can be blocked, that move is selected. 
